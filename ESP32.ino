...
//--------------Push putton (GPIO 32 and GND)
constexpr uint8_t AUDIO_BUTTON_PIN = 32;
...
bool talkMouthOpen = false;
constexpr unsigned long TALK_MOVE_MS = 130;

void talk(){
  if(audioPlaying){
    if (millis() - lastTalkMove >= TALK_MOVE_MS) {
      lastTalkMove = millis();
      talkMouthOpen = !talkMouthOpen;
      setMouthAngle(talkMouthOpen ? MOUTH_OPEN: MOUTH_CLOSED);
    }
  } else if (talkMouthOpen) {
    talkMouthOPen = false;
    setMouthAngle(MOUTH_CLOSED);
  }
...
void updateAudioButton(){
  bool reading = digitalRead(AUDIO_BUTTON_PIN);
